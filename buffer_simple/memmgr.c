...
#define MEMMGR_GET_BUFF_HEADER(B) ((buff_header_t*)(B))
#define base (buff_header->base)
#define freep (buff_header->freep)
#define pool (buff_header->pool)
#define pool_free_pos (buff_header->pool_free_pos)
#define POOL_SIZE (buff_size)

void memmgr_init(void *buff, size_t buff_size)
{
    buff_header_t *buff_header = MEMMGR_GET_BUFF_HEADER(buff);
    base.s.next = 0;
    base.s.size = 0;
    freep = 0;
    pool = (byte*) (buff_header+1);
    pool_free_pos = 0;
}


void memmgr_print_stats(void *buff, size_t buff_size)
{
    buff_header_t *buff_header = MEMMGR_GET_BUFF_HEADER(buff);
...
//
void* memmgr_alloc(void *buff, size_t buff_size, ulong nbytes)
{
    buff_header_t *buff_header = MEMMGR_GET_BUFF_HEADER(buff);
    mem_header_t* p;
    mem_header_t* prevp;
    ...
    for (p = prevp->s.next; ; prevp = p, p = p->s.next)
    {
        ...
        {
            if ((p = get_mem_from_pool(buff, buff_size, nquantas)) == 0)
            {
...
//
void memmgr_free(void *buff, size_t buff_size, void* ap)
{
    buff_header_t *buff_header = MEMMGR_GET_BUFF_HEADER(buff);
    mem_header_t* block;
    mem_header_t* p;
